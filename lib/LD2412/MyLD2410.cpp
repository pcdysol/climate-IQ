...
bool MyLD2410::processData()
{
  ...
  { // Basic mode (0x02) and Engineering mode (0x01)
    ++dataFrames;
    sData.timestamp = now;
...
{
  // LD2412: only the 0xA2 reset is needed. The original chained 0x61 (params)
  // and 0xAB (resolution) queries afterward, which the LD2412 may NAK, making
  // the whole call report failure. Drop them.
  if (isConfig)
    return sendCommand(LD2410::reset);
  return configMode() && sendCommand(LD2410::reset) && configMode(false);
}
