...
MyLD2410 library — LOCAL FORK adapted for the HLK-LD2412 (14 gates).
Based on https://github.com/iavorvel/MyLD2410 (originally for the 9-gate LD2410).

Changes vs upstream (Path A): ValuesArray widened to 14 gates, setN cap raised to 13,
and the frame buffer enlarged for the larger LD2412 engineering frame. The class/file
names are kept (MyLD2410) so existing includes and call sites are unchanged.
The engineering-frame parser is gate-count-driven (reads the gate count from the frame),
so these size changes are sufficient to stream all 14 gates of moving/stationary energy.
...
#define LD2410_BAUD_RATE 115200 // LD2412 default baud (LD2410 was 256000)
#endif
#define LD2410_BUFFER_SIZE 0x80 // enlarged for the LD2412's 14-gate engineering frame (was 0x40)
...
public:
  ...
  struct ValuesArray
  {
    byte values[14]; // LD2412 has 14 gates (0-13); LD2410 had 9 (0-8)
    byte N = 0;

    void setN(byte n)
    {
      N = (n <= 13) ? n : 13; // LD2412: max gate index 13 (was 8 for LD2410)
    }
